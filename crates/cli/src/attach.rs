...
fn run_statusline_cmd(cmd: &str, state: &AttachState) -> String {
...
async fn attach_inner(coop_url: &str, auth_token: Option<&str>, sl_cfg: &StatuslineConfig) -> i32 {
    ...
    if sl_active {
        ...
        let resize = ClientMsg::Resize { cols: init_cols, rows: content_rows };
        let _ = send_msg(&mut ws_tx, &resize).await;
    } else {
        let resize = ClientMsg::Resize { cols: init_cols, rows: init_rows };
        let _ = send_msg(&mut ws_tx, &resize).await;
    }
    ...
    if sl_active {
        let _ = send_msg(&mut ws_tx, &ClientMsg::StateRequest {}).await;
    }

    // Auth via message if token provided.
    ...
    if sl_active {
        let content = match &sl_cfg.cmd {
            Some(cmd) => run_statusline_cmd(cmd, &state),
