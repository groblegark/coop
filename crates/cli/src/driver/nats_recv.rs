        ...
        loop {
            match messages.next().await {
                Some(Ok(msg)) => {
                    ...
                    if let Some(event) = parse_nats_payload(&msg.payload) {
                        return Some(event);
                    }
...
/// Parse a NATS message payload into a [`HookEvent`].
///
/// bd daemon publishes events as JSON with `hook_event_name` as the
/// discriminator. The subject encodes the event type too (e.g. `hooks.Stop`)
/// but we parse from the payload for richer data.
pub fn parse_nats_payload(payload: &[u8]) -> Option<HookEvent> {
