...
fn acquire_http_ok() -> anyhow::Result<()> {
    ...
    let _guard = lock.acquire_http().anyhow()?;
    assert!(lock.is_held());
    Ok(())
}
...
fn acquire_ws_ok() -> anyhow::Result<()> {
    let lock = WriteLock::new();
    lock.acquire_ws("client-1").anyhow()?;
    assert!(lock.is_held());
    Ok(())
}
...
fn release_ws_then_acquire() -> anyhow::Result<()> {
    let lock = WriteLock::new();
    lock.acquire_ws("client-1").anyhow()?;
    lock.release_ws("client-1").anyhow()?;
    assert!(!lock.is_held());
    lock.acquire_ws("client-2").anyhow()?;
    Ok(())
}
